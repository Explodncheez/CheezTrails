...
public class Corruption extends ParticleTrail {

	public Corruption(ParticleInfo info) {
		super(info);
	}
    ...
    public void doMoveEffect(Player p) {
		ParticleEffects.SMOKE_LARGE.display(0.15F, 0.05F, 0.15F, 0F, 3, p.getLocation().add(0, 0.1, 0), 256);
	}
    ...
    public void doStandEffect() {
			...
			if (reverse) {
				for (Player p : users) {
					if (p.hasMetadata("trail.standingstill")) {
						ParticleEffects.LAVA.display(0.12F, 0.05F, 0.12F, 0.1F, 6, p.getLocation().add(0, 2.6, 0), 256);
						ParticleEffects.FLAME.display(0.12F, 0.05F, 0.12F, 0.12F, 12, p.getLocation().add(0, 2.7, 0), 256);
					}
				}
				return;
			}
		...
		double u = (reverse ? 1 - y : y) * 2.5;

		for (Player p : users) {
			if (p.hasMetadata("trail.standingstill")) {
				Location loc = p.getLocation().add(z, u, x);
				ParticleEffects.SMOKE.display(0.03F, 0.03F, 0.03F, 0.03F, 3, loc, 256);
			}
		}
	}
